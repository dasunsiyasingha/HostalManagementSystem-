                    ...
                    <div class="table-responsive">
                      ...
                      >
                        ...
                        <tbody>
                          ...
                          <?php
                            $sql="SELECT room.roomNo AS room_no, GROUP_CONCAT(DISTINCT student.studentID ORDER BY student.studentID SEPARATOR ', ') AS student_ids, GROUP_CONCAT(DISTINCT CONCAT(chair.chairID, IF(chair.demageState = 'damaged', ' (damaged)', '')) SEPARATOR ', ') AS chairs, GROUP_CONCAT(DISTINCT CONCAT(desk.deskID, IF(desk.demageState = 'damaged', ' (damaged)', '')) SEPARATOR ', ') AS desks, GROUP_CONCAT(DISTINCT CONCAT(bed.bedID, IF(bed.demageState = 'damaged', ' (damaged)', '')) SEPARATOR ', ') AS beds, GROUP_CONCAT(DISTINCT CONCAT(mettress.mettressID, IF(mettress.demageState = 'damaged', ' (damaged)', '')) SEPARATOR ', ') AS mattresses, GROUP_CONCAT(DISTINCT CONCAT(rack.rackID, IF(rack.demageState = 'damaged', ' (damaged)', '')) SEPARATOR ', ') AS racks, GROUP_CONCAT(DISTINCT CONCAT(locker.lockerID, IF(locker.demageState = 'damaged', ' (damaged)', '')) SEPARATOR ', ') AS lockers FROM room LEFT JOIN student ON room.roomNo = student.stRoomNo LEFT JOIN chair ON room.roomNo = chair.roomNo LEFT JOIN desk ON room.roomNo = desk.roomNo LEFT JOIN bed ON room.roomNo = bed.roomNo LEFT JOIN mettress ON room.roomNo = mettress.roomNo LEFT JOIN rack ON room.roomNo = rack.roomNo LEFT JOIN locker ON room.roomNo = locker.roomNo GROUP BY room.roomNo; ";
